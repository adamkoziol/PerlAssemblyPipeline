...
if (@trimmed_check) {
	...
	trimmomatic(\@trimmed_check, \@cpus);
} else {
...
sub fastq_check
{
	...
	my $path = getcwd;
	# Initialize
	my ($mpath, @fastq_1, @fastq_2, @fastq_folders, @fastq_1_trimmed, @fastq_2_trimmed, @trimming_incomplete, @folders);
	...
	while (defined(my $file = readdir(DIR))) {
		...
		if (-d $path."/".$file){
			...
			@fastq_1_trimmed = glob("*paired1.fastq");
			@fastq_2_trimmed = glob("*paired2.fastq");
			# If both arrays have values, then add the $path and filename to an array
			if (@fastq_1 and @fastq_2) {
				push(@fastq_folders, $path . "/" . $file);
				push(@folders, $file);	
			}
			if ((@fastq_1 and @fastq_2) and not (@fastq_1_trimmed and @fastq_2_trimmed)) {
...
sub trimmomatic
{
	...
	foreach (@files) {
		...
		system ("$path/bin/trimmomatic-0.30.jar PE -threads @cpus -phred33 -trimlog $file_name.log $fastq_1[0] $fastq_2[0] $file_name.paired1.fastq $file_name.unpaired1.fastq $file_name.paired2.fastq $file_name.unpaired2.fastq ILLUMINACLIP:$path/bin/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36");	
...
sub velvet 
{
	...
	if (length $file) {
		...
		system("perl $path/bin/VelvetOptimiser.pl -s 93 -e 99 -f '-shortPaired -fastq $file/$fastq_1[0] $file/$fastq_2[0]' -t @cpus -c 'n50*Lcon/tbp+log(Lbp)'");
		
		#system("VelvetOptimiser.pl -s 95 -e 97 -f '-shortPaired -fastq $file/$fastq_1[0] $file/$fastq_2[0]' -t @cpus -c 'n50*Lcon/tbp+log(Lbp)'");
...
sub assembly_report 
{
	...
	foreach my $folder (@folders) {
		...
		open(LOG, "<", "Logfile.txt");
			...
			if ($assembly_score > $best_assembly_score) {
				...
				# folders very easily
				$best_assembly =  ("$path/sequences/$folder/$auto_data_folder[0]");
				$best_trim = $trim
			}
	...
	foreach $assembly (@best_assemblies) {
		...
		print "\t    $folders[$count] with $number_of_contigs[$count] contigs, an N50 of $n50[$count], and an assembly size of $total_bases[$count] bases\n";
		$count++;
	} 
	close OUTPUT;
	#chdir ("$path");
